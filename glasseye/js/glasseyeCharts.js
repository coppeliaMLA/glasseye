...
var TimeSeries = function(processed_data, div, size, labels, scales) {
  ...
  GridChart.call(this, div, size, labels, scales, margin);
  ...
  //Some customisations
  this.y_axis.tickFormat("");
  this.x_axis.tickFormat(d3.time.format("%Y-%m"));
...
TimeSeries.prototype.add_timeseries = function() {
  ...
  this.chart_area.selectAll(".groups")
    .data(this.processed_data)
    .enter()
    .append("g")
    .append("path")
    ...
    .attr("d", function(d) {
      return self.line(d.values.filter(function(e) {
        return e.variable === "absolute"
      }));
    })

  this.create_linepoints("absolute");
...
TimeSeries.prototype.create_linepoints = function(to_variable) {
  ...
  var line_points = this.processed_data.map(function(d) {

    return (d.values.map(function(e) {
      if (e.variable === to_variable) {
        return {
          time: e.time,
          value: e.value,
          group: d.group
...
TimeSeries.prototype.flip_variable = function(to_variable) {

  //Update y axis
  this.y.domain(minmax_across_groups(this.processed_data, to_variable));


  //Update paths
  this.chart_area.selectAll(".timeseries_line")
    .transition()
    .delay(1000)
    .duration(1000)
    .attr("d", function(d) {
  ...
  this.chart_area.selectAll('.line_points').remove();

  //Create the line point data

  this.create_linepoints(to_variable);
...
var Thermometers = function(processed_data, div, size, labels, scales) {
  ...
  BarChart.call(this, processed_data, div, size, labels, scales);
  this.bar_width = 30;
...
Thermometers.prototype.add_thermometers = function() {
  ...
  var start_date = new Date("Wed Mar 31 2010 00:00:00 GMT+0100 (BST)");

  console.log(this.processed_data.filter(function(d){return d.time.getTime() === start_date.getTime() & d.filter === "aerial"}));

  var therm = this.chart_area.selectAll(".thermometer")
    .data(this.processed_data.filter(function(d){return d.time.getTime() === start_date.getTime() & d.filter === "aerial"}))
    .enter()
    .append("g")
    .attr("class", "thermometer")
    .attr("transform", function(d) {return "translate(" + (self.x(d.category) - self.bar_width / 4) + ", " + 0 +  ")"})
    .on('mouseover', self.tip.show)
    .on('mouseout', self.tip.hide);
...
/*
This section contains the functions that create the charts from either a csv file
or from inline data
*/
...
function timeseries(data, div, size, labels) {
  ...
  var draw = function draw_timeseries(processed_data, div, size, labels) {
    ...
    var glasseye_chart = new TimeSeries(processed_data, div, size, labels, scales);
    glasseye_chart.add_svg().add_grid().add_timeseries().flip_variable("indexed").flip_variable("share");
...
/*
This section contains the functions that are used by the chart building functions
*/
